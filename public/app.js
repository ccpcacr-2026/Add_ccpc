...
/* ─── Defaults ──────────────────────────────────────── */
// DEFAULT_FORM, DEFAULT_ADMIT + the print generators live in print-templates.js
// (shared with the applicant print page) — loaded before this file in app.html.
...
// fmtDate + deepMerge come from print-templates.js (shared, loaded first)
...
function showAdminTab(id,btn){
  ...
  if(id==='at-payments')loadPaymentsTab();
}
...
function renderTable(apps){
  ...
  tbody.innerHTML=apps.map(a=>`
    <tr class="border-t border-slate-50 hover:bg-slate-50/80 transition-all cursor-pointer" onclick="loadForm(${a.id})">
      ...
      <td class="px-4 py-3"><p class="font-bold text-sm text-slate-800">${a.name_english||'—'}</p><p class="text-[10px] text-slate-400">${a.name_bangla||''}</p>
        <p class="text-[9px] mt-0.5">${payChip(a)}${a.room_no?` <span class="inline-block px-1.5 py-0.5 rounded bg-violet-100 text-violet-700 font-bold">Room ${a.room_no}</span>`:''}</p></td>
...
/* openPrintTab, generateTablesHtml, generateFormHtml, generateAdmitHtml
   moved to print-templates.js (shared with the applicant print page). */
  ...
  host.innerHTML=c.classes.map(function(cl){
    ...
    return '<div style="border:1px solid #e2e8f0;border-radius:10px;padding:10px 12px;margin-bottom:8px;background:#fff">'+
      ...
      '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-top:6px"><span style="font-size:10px;font-weight:700;color:#64748b;min-width:64px">Seats:</span>'+
        '<input class="finput finput-sm" style="width:100px" value="'+circEscH(cl.seats||'')+'" oninput="circSetSeats(\''+circEscH(cl.class)+'\',this.value)" placeholder="e.g. 50">'+
        '<span style="font-size:10px;font-weight:700;color:#64748b">Fee (BDT):</span>'+
        '<input class="finput finput-sm" type="number" style="width:100px" value="'+circEscH(cl.fee||'')+'" oninput="circSetFee(\''+circEscH(cl.class)+'\',this.value)" placeholder="default"></div>'+
    '</div>';
  }).join('');
}
...
function circSetFee(cls,val){const c=circFindClass(cls);if(c)c.fee=val;}
...
/* ═══════════════════════════════════════════════════
   PAYMENTS & ADMIT CARDS (admin tab)
   ═══════════════════════════════════════════════════ */
function payChip(a){
  if(a.payment_status==='verified')return '<span class="inline-block px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 font-bold">Paid ✓</span>';
  if(a.payment_status==='review')return '<span class="inline-block px-1.5 py-0.5 rounded bg-amber-100 text-amber-700 font-bold">Pay review</span>';
  if(a.stage==='submitted')return '<span class="inline-block px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 font-bold">Unpaid</span>';
  if(a.stage==='initiated')return '<span class="inline-block px-1.5 py-0.5 rounded bg-slate-100 text-slate-500 font-bold">Draft</span>';
  return '';
}

let currentPaySettings=null;
async function loadPaymentsTab(){
  setLoading(true);
  const r=await api('getSettings',{});
  setLoading(false);
  const s=(r.settings||{}).payment_settings||{};
  currentPaySettings=s;
  setV('ps-store-id',s.sslStoreId||'');
  setV('ps-store-pass',s.sslStorePass||'');
  setV('ps-default-fee',s.defaultFee||'');
  setChk('ps-sandbox',s.sslSandbox!==false);
  setChk('ps-gateway-on',s.gatewayEnabled!==false);
  setChk('ps-manual-on',s.manualEnabled!==false);
  setV('ps-manual-numbers',s.manualNumbers||'');
  setV('ps-manual-instructions',s.manualInstructions||'');
  loadPaymentsList();
}
async function savePaymentSettings(){
  const value={
    sslStoreId:v('ps-store-id'), sslStorePass:v('ps-store-pass'),
    defaultFee:v('ps-default-fee'), currency:'BDT',
    sslSandbox:chk('ps-sandbox'), gatewayEnabled:chk('ps-gateway-on'),
    manualEnabled:chk('ps-manual-on'),
    manualNumbers:v('ps-manual-numbers'), manualInstructions:v('ps-manual-instructions'),
  };
  setLoading(true);
  const r=await api('saveSettings',{key:'payment_settings',value});
  setLoading(false);
  if(r.error){toast(r.error,'error');return;}
  currentPaySettings=value;
  toast('Payment settings saved'+(value.sslStoreId?(value.sslSandbox?' — gateway in SANDBOX mode':' — gateway LIVE'):' — gateway off (no Store ID)'),'success');
}

const PAY_STATUS_COLORS={initiated:'bg-slate-100 text-slate-500',review:'bg-amber-100 text-amber-700',valid:'bg-emerald-100 text-emerald-700',verified:'bg-emerald-100 text-emerald-700',failed:'bg-red-100 text-red-600',cancelled:'bg-slate-100 text-slate-400',rejected:'bg-red-100 text-red-600'};
async function loadPaymentsList(){
  const host=document.getElementById('payments-table');if(!host)return;
  host.innerHTML='Loading…';
  const status=(document.getElementById('pay-filter')||{}).value||'';
  const r=await api('listPayments',status?{status}:{});
  const pays=r.payments||[];
  if(!pays.length){host.innerHTML='<p class="text-xs text-slate-400 py-4">No payments'+(status?' with this status':'')+'.</p>';return;}
  host.innerHTML='<table class="w-full text-left" style="font-size:12px"><thead><tr class="text-[10px] uppercase text-slate-400">'+
    '<th class="px-2 py-2">When</th><th class="px-2 py-2">Applicant</th><th class="px-2 py-2">Gateway</th><th class="px-2 py-2">Ref / TrxID</th><th class="px-2 py-2">Amount</th><th class="px-2 py-2">Status</th><th class="px-2 py-2"></th>'+
    '</tr></thead><tbody>'+pays.map(p=>{
      const a=p.application||{};
      const trx=(p.raw&&p.raw.trxId)||p.val_id||p.tran_id||'';
      const when=p.created_at?new Date(p.created_at).toLocaleString():'';
      const act=p.status==='review'
        ?'<button class="nav-btn" style="font-size:9px;color:#059669" onclick="verifyPay('+p.id+',true)">✓ Verify</button> <button class="nav-btn" style="font-size:9px;color:#dc2626" onclick="verifyPay('+p.id+',false)">✕ Reject</button>'
        :'';
      return '<tr class="border-t border-slate-100">'+
        '<td class="px-2 py-2 text-slate-400 whitespace-nowrap">'+when+'</td>'+
        '<td class="px-2 py-2"><b>'+(a.name_english||'—')+'</b><br><span class="font-mono text-[10px] text-blue-600">'+(a.tracking_id||'')+'</span> <span class="text-[10px] text-slate-400">'+(a.index_id||'')+'</span></td>'+
        '<td class="px-2 py-2">'+(p.gateway==='manual'?((p.raw&&p.raw.method)||'manual')+(p.payer_ref?'<br><span class="text-[10px] text-slate-400">'+p.payer_ref+'</span>':''):p.gateway)+'</td>'+
        '<td class="px-2 py-2 font-mono text-[11px]">'+trx+'</td>'+
        '<td class="px-2 py-2 font-bold">৳'+(p.amount||0)+'</td>'+
        '<td class="px-2 py-2"><span class="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold '+(PAY_STATUS_COLORS[p.status]||'bg-slate-100 text-slate-500')+'">'+p.status+'</span></td>'+
        '<td class="px-2 py-2 whitespace-nowrap text-right">'+act+'</td></tr>';
    }).join('')+'</tbody></table>';
}
async function verifyPay(pid,approve){
  if(!approve&&!(await openConfirm('Reject this payment? The applicant will see it as rejected and can retry.')))return;
  setLoading(true);
  const r=await api('verifyPayment',{paymentId:pid,approve});
  setLoading(false);
  if(r.error){toast(r.error,'error');return;}
  toast(approve?'Payment verified — form download unlocked':'Payment rejected','success');
  loadPaymentsList();
}
async function markPaidByKey(){
  const key=v('markpaid-key').toUpperCase();
  if(!key){toast('Enter a tracking or index number','warn');return;}
  const r=await api('listApplications',{});
  const apps=r.applications||[];
  const found=apps.find(a=>String(a.tracking_id||'').toUpperCase()===key||String(a.index_id||'').toUpperCase()===key);
  if(!found){toast('No application matches '+key,'error');return;}
  if(!(await openConfirm('Mark '+(found.name_english||key)+' ('+found.tracking_id+') as PAID (offline)?')))return;
  setLoading(true);
  const r2=await api('markPaid',{appId:found.id});
  setLoading(false);
...
/* ── Admit issue: CSV / Excel / paste → [{key, room}] ── */
let admitRows=[];
function _admitSetRows(rows){
  admitRows=rows.filter(r=>r.key&&r.room);
  const prev=document.getElementById('admit-preview');
  const btn=document.getElementById('admit-issue-btn');
  if(!admitRows.length){prev.innerHTML='';btn.disabled=true;return;}
  const shown=admitRows.slice(0,8);
  prev.innerHTML='<div class="text-xs text-slate-500 mb-1"><b>'+admitRows.length+'</b> row'+(admitRows.length>1?'s':'')+' ready:</div>'+
    '<table class="text-xs font-mono"><tbody>'+shown.map(r=>'<tr><td class="pr-6 py-0.5 text-blue-600">'+r.key+'</td><td class="py-0.5">'+r.room+'</td></tr>').join('')+
    (admitRows.length>shown.length?'<tr><td colspan="2" class="text-slate-400 py-0.5">…and '+(admitRows.length-shown.length)+' more</td></tr>':'')+
    '</tbody></table>';
  btn.disabled=false;
}
function _admitParsePairs(rows2d){
  // rows2d: array of arrays (first two non-empty cells = key, room); skips a header row
  const out=[];
  rows2d.forEach(function(cells,i){
    const vals=(cells||[]).map(function(c){return String(c==null?'':c).trim();}).filter(function(x){return x!=='';});
    if(vals.length<2)return;
    if(i===0&&/track|index|key|id/i.test(vals[0])&&/room/i.test(vals[1]))return; // header
    out.push({key:vals[0],room:vals[1]});
  });
  return out;
}
function admitPasteChanged(){
  const txt=v('admit-paste');
  const rows2d=txt.split(/\r?\n/).map(function(l){return l.split(/[,\t;]/);});
  _admitSetRows(_admitParsePairs(rows2d));
}
function admitFileChosen(input){
  ...
  rd.onload=function(){
    try{
      let rows2d;
      if(isCsv){
        rows2d=String(rd.result).split(/\r?\n/).map(function(l){return l.split(/[,\t;]/).map(function(c){return c.replace(/^"|"$/g,'');});});
      }else{
        if(typeof XLSX==='undefined'){toast('Excel library not loaded — use CSV or paste','error');return;}
        const wb=XLSX.read(rd.result,{type:'array'});
        rows2d=XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]],{header:1,raw:false});
      }
      _admitSetRows(_admitParsePairs(rows2d));
      const n=admitRows.length;
      document.getElementById('admit-issue-note').textContent=n?('Parsed '+n+' rows from '+file.name):'No usable rows found — expected columns: key, room';
    }catch(e){toast('Could not read that file: '+e.message,'error');}
  };
  if(isCsv)rd.readAsText(file);else rd.readAsArrayBuffer(file);
}
async function issueAdmitsGo(){
  if(!admitRows.length)return;
  if(!(await openConfirm('Issue admit cards for '+admitRows.length+' application(s)? Matching applicants can immediately download their admit card with the room number.')))return;
  setLoading(true);
  const r=await api('issueAdmits',{rows:admitRows});
  setLoading(false);
  if(r.error){toast(r.error,'error');return;}
  const note=document.getElementById('admit-issue-note');
  note.textContent='Issued: '+r.updated+' · Matched: '+r.matched+(r.unmatched&&r.unmatched.length?' · Unmatched: '+r.unmatched.join(', '):'');
  toast('Admit cards issued for '+r.updated+' application(s)','success');
  if(r.unmatched&&r.unmatched.length)toast(r.unmatched.length+' key(s) did not match any application','warn');
}
